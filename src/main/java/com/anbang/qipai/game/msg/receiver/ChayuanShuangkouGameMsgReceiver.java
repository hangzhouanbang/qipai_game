...
import com.anbang.qipai.game.msg.channel.sink.ChayuanShuangkouGameSink;
	...
	public void receive(CommonMO mo) {
		...
		if ("playerQuit".equals(msg)) {// 有人退出游戏
			...
			if (room != null) {
				...
				gameService.chayuanShuangkouPlayerQuitQame(gameId, playerId);
			}
		}
		if ("ju canceled".equals(msg)) {// 取消游戏
			Map data = (Map) mo.getData();
			String gameId = (String) data.get("gameId");
			GameRoom gameRoom = gameService.findRoomByGameAndServerGameGameId(Game.chayuanShuangkou, gameId);
			if (gameRoom != null) {
		...
		if ("game delay".equals(msg)) {// 游戏延时
			...
			GameRoom gameRoom = gameService.findRoomByGameAndServerGameGameId(Game.chayuanShuangkou, gameId);
			// 延时11小时
			gameService.delayGameRoom(Game.chayuanShuangkou, gameId, gameRoom.getDeadlineTime() + 11 * 60 * 60 * 1000);
		}
	}
}
