...
import com.anbang.qipai.game.msg.channel.sink.DaboluoGameSink;
	...
	public void receive(CommonMO mo) {
		...
		if ("playerQuit".equals(msg)) {// 有人退出游戏
			...
			if (room != null) {
				...
				gameService.daboluoPlayerQuitQame(gameId, playerId);
			}
		}
		if ("ju canceled".equals(msg)) {// 取消游戏
			Map data = (Map) mo.getData();
			String gameId = (String) data.get("gameId");
			GameRoom gameRoom = gameService.findRoomByGameAndServerGameGameId(Game.daboluo, gameId);
			if (gameRoom != null) {
		...
		if ("game delay".equals(msg)) {// 游戏延时
			...
			GameRoom gameRoom = gameService.findRoomByGameAndServerGameGameId(Game.daboluo, gameId);
			// 延时11小时
			gameService.delayGameRoom(Game.daboluo, gameId, gameRoom.getDeadlineTime() + 11 * 60 * 60 * 1000);
		}
	}
}
